...
public class JdbcExchangeConnection implements CrudManager<ExchangeRate>{
    ...
    public int create(ExchangeRate entity) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connect.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            preparedStatement.setDouble(3, formatter.roundToTwoDecimalPlaces(entity.getRate()));
    ...
    public void update(ExchangeRate entity) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connect.prepareStatement(sql)) {
            connect.setAutoCommit(false);
            preparedStatement.setDouble(1, formatter.roundToTwoDecimalPlaces(entity.getRate()));
    ...
    public double findRateByUsdBase(String baseCurrency, String targetCurrency) throws SQLException {
        ...
        if (baseToUsdRate.isPresent()) {
            if (targetToUsdRate.isPresent()) {
                return formatter.roundToTwoDecimalPlaces(baseToUsdRate.get().getRate() / targetToUsdRate.get().getRate())   ;
            } else if (usdToTargetRate.isPresent()) {
                return formatter.roundToTwoDecimalPlaces(baseToUsdRate.get().getRate() * usdToTargetRate.get().getRate());
            }
        } else if (usdToBaseRate.isPresent()) {
            if (targetToUsdRate.isPresent()) {
                return formatter.roundToTwoDecimalPlaces(targetToUsdRate.get().getRate() / usdToBaseRate.get().getRate());
            } else if (usdToTargetRate.isPresent()) {
                return formatter.roundToTwoDecimalPlaces(1 / (usdToBaseRate.get().getRate() * usdToTargetRate.get().getRate()));
            }
        }
